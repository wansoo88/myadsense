...
2. **근접 중복 차단**: 기존·신규 페이지 간 유사도(k=5 단어 shingle 의 Jaccard) 임계 초과 = 거부.
   구현 `quality_gate._shingles` · 임계 `config/content.yaml near_duplicate.max_similarity: 0.70`.
   > 🔴 **이게 막는 것과 못 막는 것 (2026-08-01 실측 — 추정 아님).**
   > 이 검사는 **축자 복제 탐지기**다. **"같은 주제를 다시 쓴 글"은 잡지 못한다.**
   > 실제 발행분 1편(2,371단어)으로 잰 값:
   >
   > | 원문 대비 변형 | 유사도 | 임계 0.70 판정 |
   > |---|---|---|
   > | 동일 문서 | 1.0000 | 거부 |
   > | 단어 **90% 유지**(10%만 무작위 삭제) | **0.4671** | **통과** |
   > | 단어 70% 유지 | 0.1302 | 통과 |
   > | 단어 50% 유지 | 0.0420 | 통과 |
   >
   > 즉 **10%만 달라져도 이미 임계의 3분의 2 아래**다. LLM 이 같은 주제를 새로 쓰면 문장이 전부
   > 달라지므로 사실상 절대 걸리지 않는다. (오탐도 없다 — 무관한 기존 32편과의 최고 유사도는 0.0436.)
   >
   > **함의**: "근접중복 가드가 주제 중복을 막아줄 것"이라는 가정에 기대어 설계·지시하지 말 것.
   > 실제로 그 가정 때문에 ORDER 51-ops 후속 지시가 한 번 어긋났다(승인 대기 중인 시범편과 같은
   > 주제가 자동 재생성될 뻔했다). 주제 단위 중복은 **다른 장치**로 막아야 한다 —
   > `published.json` 키워드 스킵, 후보 목록 관리, 또는 `dist/pending_approval` 주제 제외(예정).
...
구동: **보유 Ubuntu 서버의 cron으로 24/7 실행** (`config/sites.yaml` deploy). 서버: `ssh -i ~/.ssh/autobtc_iwinv root@115.68.230.40`, 도메인 `utilverse.info` 서브도메인. **웹서버는 nginx+certbot**(80/443, data·itsmine 등 공유 — Caddy 아님). `stack.utilverse.info` 라이브(정적 vhost+TLS 1회 셋업 완료, 실콘텐츠 15편 배포·noindex 해제·색인 허용 — 2026-07-02 확인). 콘텐츠 배포는 `engine/deploy.py`(tar/scp over ssh). `config/schedule.yaml` → 서버 `crontab`. 보조로 Claude Code 예약 에이전트(`/loop`)·스킬로 점검·디버그. ⚠️ 버전관리·예약 클라우드 에이전트는 **`git init` 후** 가능(이전 ultraplan이 git 부재로 실패).
